...
export default function Footer({ block, locale }: FooterProps) {
  const isRTL = locale === 'ar';
  
  // Default settings if no block is provided or block is disabled
  const defaultSettings = {
    companyInfo: {
      name: isRTL ? 'الصفحات الخضراء' : 'Green Pages',
      description: isRTL 
        ? 'دليل رسمي موثوق للأنشطة التجارية والخدمية'
        : 'Official trusted directory for businesses and services',
      logo: true,
    },
    sections: [
      {
    ...
    copyright: isRTL 
      ...
      : `© ${new Date().getFullYear()} Green Pages. All rights reserved.`,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-bold">{settings.companyInfo.name}</span>
            </div>
            ...
            {settings.socialMedia.show && (
              <div className="flex gap-4">
                {settings.socialMedia.links.map((social: any, index: number) => (
                  <a
                    key={index}
                    href={social.url}
                    target="_blank"
          ...
          {/* Footer Sections */}
          {settings.sections.map((section: any, sectionIndex: number) => (
