...
import { DistrictDialog } from '@/components/geo/district-dialog';
...
export default function DistrictsPage() {
  ...
  const loadDistricts = async () => {
    try {
      setLoading(true);
      const data = await apiClient.get<District[]>('/geo/districts?locale=ar');
      setDistricts(data);
    } catch (error) {
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">إدارة الأحياء في النظام</p>
          </div>
        ...
        <Card>
          ...
          <CardContent>
            <AdminDataTable
              ...
              loading={loading}
              searchable
              searchPlaceholder="البحث في الأحياء..."
              actions={(item) => (
                <div className="flex items-center gap-2">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => router.push(`/geo/districts/${item.id}`)}
                  >
