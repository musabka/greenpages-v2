...
import { GovernorateDialog } from '@/components/geo/governorate-dialog';
...
export default function GovernoratesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await apiClient.delete(`/geo/governorates/${id}`);
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">إدارة المحافظات في النظام</p>
          </div>
              ...
              actions={(item) => (
                <div className="flex items-center gap-2">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => router.push(`/geo/governorates/${item.id}`)}
                  >
